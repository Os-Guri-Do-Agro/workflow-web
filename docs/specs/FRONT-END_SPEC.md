...
>
> Features cobertas: Gestão existente (atividades/kanban, quarters, tickets,
> bug-report com IA, notes, GitHub, Discord), Calendário, Roadmap mensal,
> **Boards (canvas Yjs realtime)**, **Workspace Intelligence (busca/IA)**,
> **Share links públicos**, **Inbox**, **Colaboração
> (comentários/menções/reações/feed/digest)**, Realtime, Export PDF e Health.
...
- **Erros (envelope padronizado):** TODA resposta de erro vem como
  `{ statusCode, error, message, path, timestamp, requestId }`.
  - `message` pode ser **string OU array** (array nos 400 de validação) → exibir
    `Array.isArray(m) ? m.join(', ') : m`.
  - `error` é a reason-phrase (ex.: `"Not Found"`, `"Forbidden"`, `"Bad Request"`).
  - `requestId` também volta no header **`x-request-id`** (exposto via CORS) —
    logue-o no front; facilita rastrear o erro no backend.
- **Rate limit:** rotas de IA são limitadas → podem responder **`429`** (Too Many
  Requests) com `Retry-After`. Limites: `/copilot/*` 10/60s (roadmap 5/60s),
  `/search` 15/60s. Trate 429 com backoff/aviso; não faça retry agressivo.
...
- **Validação permissiva:** o backend ignora campos extras do payload (não dá
  erro por mandar a mais) e coage tipos; payload realmente inválido → `400` com
  `message` array dizendo o que está errado.
...
```typescript
// Reutilizado em feed/comments/inbox (autor/ator resolvido pelo backend):
...
export interface AppNotification {
  ...
  createdAt: string
  userId: string
  user: PublicUser | null // dono da notificação (resolvido)
}
```
...
export interface Comment {
  ...
  entityId: string
  authorId: string
  author: PublicUser | null // resolvido pelo backend
  body: string
  mentions: string[]
  createdAt: string
  updatedAt: string
  reactions: { id: string; userId: string; emoji: string; user: PublicUser | null }[]
}
export interface FeedEvent {
  id: string
  actorId: string
  actor: PublicUser | null // resolvido pelo backend
...
> **Autor/ator já resolvido:** o backend devolve `author`/`actor`/`user`
> (`{id,name,email}` ou `null`) junto, batch — o front **não precisa** buscar o
> usuário por id. Os campos `*Id` continuam presentes.
...
## 8. Realtime (socket.io) — push ao vivo + presença

Fim do polling: o backend empurra eventos via **socket.io** no path `/socket.io`
(mesma porta/host). Coexiste com o WS `/collab` (Yjs) — são canais diferentes.
...
const socket = io(import.meta.env.VITE_API_URL, { auth: { token: jwt } })
// rooms automáticos: company:<id> de cada empresa do token + user:<id>

socket.on('notification:new', (n) => {
  /* badge++, prepend no inbox */
})
socket.on('feed:new', (e) => {
  /* prepend na timeline */
})
socket.on('comment:new', (c) => {
  /* append no thread da entidade (c já traz author) */
})
socket.on('presence:update', ({ companyId, online }) => {
  /* online: string[] de userIds */
})
```

| Evento             | Room    | Payload                              |
| ------------------ | ------- | ------------------------------------ |
| `notification:new` | user    | `AppNotification` (nova notificação) |
| `feed:new`         | company | `FeedEvent`                          |
| `comment:new`      | company | `Comment` (com `author`)             |
| `presence:update`  | company | `{ companyId, online: string[] }`    |

- **Auth:** passe o **mesmo JWT** em `auth.token`. Sem token válido → desconecta.
- **Presença inicial:** `GET /presence` (REST, com `x-company-id`) → `{ online: string[] }`.
  Depois mantenha via `presence:update`.
- **Reconexão:** o socket.io reconecta sozinho. Re-sincronize estado no `connect`.
- Com isso, o **sininho do inbox e a timeline atualizam ao vivo** — pode remover o polling.

---

## 9. Export PDF

`GET /export/roadmap.pdf?year=` (membro, `x-company-id`) → baixa o **PDF do
roadmap anual** (download direto; `Content-Disposition: attachment`). Gerado no
servidor (pdfkit). Use num botão "Exportar PDF" na tela de roadmap.
...
## 11. Catálogo rápido de endpoints (resumo)
...
Colab:     GET/POST /comments · PATCH/DELETE /comments/:id · POST/DELETE /comments/:id/reactions[/:emoji]
           GET /feed · POST /copilot/digest
Realtime:  socket.io /socket.io (auth.token=jwt) · eventos: notification:new, feed:new, comment:new, presence:update
Presence:  GET /presence
Export:    GET /export/roadmap.pdf?year=
Health:    GET /health
```

---

## 12. Checklist macro de telas
...
## 13. Notas finais p/ o Cursor

- Tudo company-scoped via `x-company-id` (menos eventos pessoais e `/public/*`).
- Autor/ator/usuário já vem resolvido nos payloads (`author`, `actor`, `user`
  como `{ id, name, email } | null`); IDs como `authorId`, `actorId`,
  `mentions` e `createdById` continuam presentes para relacionamento e ações.
